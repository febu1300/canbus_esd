...
/*                                                                      */
/*          Copyright 1997 - 2022 by esd electronics gmbh               */
/*----------------------------------------------------------------------*/
/*                                                                      */
/*      Filename:      cantest.c                                        */
/*      Date:          1999-11-09                                       */
/*      Language:      ANSI C                                           */
/*      Targetsystem:  Win 11/10/8/7/Vista/XP/2K/NT, Win 9x/ME, WinCE,  */
/*                     QNX, VxWorks, UNIX, Linux (RTAI/RT), NET+OS,     */
/*                     RTOS-UH, RTX/RTX64, OnTime RTOS-32               */
/*                                                                      */
...
/* Revision history:                                                    */
/*----------------------------------------------------------------------*/
/* 314,25may22,ot  * Added support to configure TDC setup               */
/*                 * Show configured TDC filter value                   */
/* 313,12may22,ot  * Decode feature DAR support                         */
/*                 * Transmit data with DAR enabled if a negative value */
/*                 *  for Tx timeout is given for test 1,21,51,61       */
/* 312,08apr22,ot  * Show configured TX TS Window size                  */
/* 311,02feb22,stm * Fixed output for FW UPDATE required. If a CAN net  */
/*                 * needed a FW update this condition was also flagged */
/*                 * incorrectly for all following CAN networks.        */
/* 310,26nov21,ot  * Show TDC configuration with test 84 based on       */
/*                 *  enhanced canGetBaudrateX() capabilities           */
/* 30F,23aug21,ot  * Added plain text for some more controller types    */
/*                 * Decode feature Tx Pause                            */
/*                 * Use seperator '.'/',' between (after) bitrate to   */
/*                 *  enable listen-only / self-test mode               */
/*                 * Show internal SW timestamp tick frequency          */
/*                 * Improve TDC information presentation               */
/* 30E,04mar21,ot  * Show DLC instead of payload size for Classical CAN */
/*                 *  messages in print_cmsg() / print_cmsg_t()         */
/*                 * Added plain text for some more controller and      */
/*                 *  transceiver types                                 */
...
/*                 *  for parameter baudrate to configure autobaud,     */
...
static char* rcsid __attribute__((unused)) = "$Id: cantest.c 17263 2022-05-30 10:07:47Z oliver $";
#else  /* No or old GNU compiler */
# define USE(var) static void use_##var(void *x) {if(x) use_##var((void *)var);}
static char* rcsid = "$Id: cantest.c 17263 2022-05-30 10:07:47Z oliver $";
USE(rcsid);
...
static void set_num_baudrate(uint32_t baud, uint32_t baudData) {
    ...
    case NTCAN_BAUD_4000:   num_baudrate_data = 4000000; break;
...
static int baudx_initialize(NTCAN_BAUDRATE_X *pBaud, uint32_t baudNominal,
                            uint32_t baudData, uint32_t tdc_cfg)
{
    ...
    if (tdc_cfg != 0xFFFFFFFF) {
        pBaud->flags |= NTCAN_BAUDRATE_FLAG_TDC;
        pBaud->tdc.tdc_mode = (uint8_t)(tdc_cfg & 0x03);
        if (NTCAN_TDC_MODE_AUTO == pBaud->tdc.tdc_mode) {
            pBaud->tdc.ssp_shift = (int8_t)((tdc_cfg >> 8) & 0x7F);
        } else if (NTCAN_TDC_MODE_MANUAL == pBaud->tdc.tdc_mode) {
            pBaud->tdc.ssp_offset = (uint8_t)((tdc_cfg >> 8) & 0x7F);
        }
        pBaud->tdc.tdc_filter = (uint8_t)((tdc_cfg >> 16) & 0x7f);
    }
...
# endif /* of _WIN32_WCE */
  ...
  if (argc > 8) {           /* Parameter 'txtout' */
    txtout = atoi(argv[8]);
    if ((txtout < 0) && (1 == test || 21 == test || 51 == test || 61 == test)) {
        txtout *= -1;
        tx_flags |= NTCAN_DAR;
        printf("Enable DAR mode for test %d\n", test);
    }
  }
  ...
  if (argc > 10) {           /* Parameter 'baudrate' */
      uint8_t baud_lom = 0, baud_stm = 0;

      /*
       * Enable listen only mode for separator '.' and self-test mode
       * for seperator ','
       */
      if (strchr(argv[10], '.') != NULL) {
          baud_lom = 1;
      } else if (strchr(argv[10], ',') != NULL) {
          baud_stm = 1;
      }
      if(parse_baud_arg(argv[10], &baudrate, &baudData, &tdc) != 0) {
          printf("!! Parameter 'baudrate' invalid !!\n");
          CANTEST_FREE(pCtx);
          return(-1);
      }
      if (baud_lom != 0) {
          baudrate |= NTCAN_LISTEN_ONLY_MODE;
      }
      if (baud_stm != 0) {
          baudrate |= NTCAN_SELF_TEST_MODE;
      }
  }
  ...
  for (i = 0; i < MAX_TX_MSG_CNT; i++) {
    ...
    *((uint32_t*)(&cmt->data[4])) = k;
    ...
    cmx = &pCtx->txmsg_x[i];
    cmx->id = idstart;
    cmx->len = (uint8_t)(NTCAN_FD | NTCAN_DATASIZE_TO_DLC(64) | tx_flags);
  ...
  */
  ...
  if ( (test != 5) && (test != 53) && (baudrate!=0xFFFFFFFFU) ) {
      ...
      if((pCtx->mode & NTCAN_MODE_FD) != 0) {
          baudx_initialize(&baudX, baudrate, baudData, tdc);
    ...
    case 84:
      {
          ...
          if (NTCAN_SUCCESS == bitrate.valid) {
              ...
              long      tq;
              uint32_t  tdc_data;
              NTCAN_TDC_CFG tdc_cfg = { 0xFF, 0x00, 0x00, 0x00 };

              /*
               * Preset TDC configuration with data from IOCTL in case canGetBaudrateX()
               * does not yet support returning the TDC configuration.
               */
              if(NTCAN_SUCCESS == canIoctl(h0, NTCAN_IOCTL_GET_FD_TDC, &tdc_data)) {
                  tdc_cfg.tdc_mode = NTCAN_GET_TDC_MODE(tdc_data);
                  if (NTCAN_TDC_MODE_AUTO == NTCAN_GET_TDC_MODE(tdc_data)) {
                      tdc_cfg.ssp_shift = NTCAN_GET_TDC_SSPS(tdc_data);
                  } else if (NTCAN_TDC_MODE_MANUAL == NTCAN_GET_TDC_MODE(tdc_data)) {
                      tdc_cfg.ssp_offset = NTCAN_GET_TDC_SSPS(tdc_data);
                  }
                  tdc_cfg.tdc_filter = NTCAN_GET_TDC_FILTER(tdc_data);
              }
              ...
              if(NTCAN_BAUD_FD == bitrate.baud) {
                  ...
                  printf("Additional flags                     : 0x%08lX\n",
                      (long)bitrate.flags);
                  ret = canGetBaudrateX(h0, &baudX);
                  if (NTCAN_SUCCESS == ret) {
                      printf("Transmitter Delay Compensation       : ");
                      if (baudX.flags & NTCAN_BAUDRATE_FLAG_TDC) {
                          tdc_cfg = baudX.tdc;
                      }
					  switch (tdc_cfg.tdc_mode) {
					  case NTCAN_TDC_MODE_AUTO:
						  printf("Automatic (Shift/Filter) %d/%d mtq\n",
							  tdc_cfg.ssp_shift, tdc_cfg.tdc_filter);
						  break;
					  case NTCAN_TDC_MODE_MANUAL:
						  printf("Manual (Offset/Filter) %d/%d mtq\n",
                              tdc_cfg.ssp_offset, tdc_cfg.tdc_filter);
						  break;
					  case NTCAN_TDC_MODE_OFF:
						  printf("Disabled\n");
						  break;
					  default:
						  printf("Invalid TDC Mode ???\n");
					  }
                  }
              }
              else
...
static void help(int testnr)
{
  ...
  printf("CAN Test%s Rev %d.%d.%d  -- (c) 1997 - 2022 esd electronics gmbh\n\n",
#if defined(NTCAN_FD)
         " FD",
#else
         "",
    ...
    if (ret == NTCAN_SUCCESS) {
      ...
      if (ret != NTCAN_SUCCESS) {
...
#if defined(NTCAN_FD)
                if((NTCAN_BAUD_FD == baudrate) && (bitrate.rate_d != 0)) {
        ...
        if (-3 == testnr) {
            ...
            printf("     %c Tx Pause          %c DAR support (%c)\n",
                features & NTCAN_FEATURE_TX_PAUSE ? '+' : '-',
                features & NTCAN_FEATURE_DAR ? '+' : '-',
                features & NTCAN_FEATURE_DAR_FRAME ? 'F' : 'G');
       }
        ...
        switch(NTCAN_GET_CTRL_TYPE(cstat.boardstatus)) {
            ...
            case NTCAN_CANCTL_MCP2515: printf("Microchip MCP2515"); break;
            case NTCAN_CANCTL_MCAN:    printf("Bosch IP for CAN FD (M_CAN)"); break;
            case NTCAN_CANCTL_CAST:    printf("CAST IP for Classical CAN"); break;
            case NTCAN_CANCTL_ESDLIN:  printf("esd IP core for LIN"); break;
            case NTCAN_CANCTL_MSAM:    printf("Microchip SAM x70/V71"); break;
            default:
        ...
        case NTCAN_TRX_SN65HVD230: printf("TI SN65HVD230"); break;
        default:
        ...
        if ((-3 == testnr) && (features & NTCAN_FEATURE_CAN_FD)) {
            uint32_t tdc_config;
            ...
            if (NTCAN_SUCCESS == ret) {
                ...
                {
                case NTCAN_TDC_MODE_AUTO:
                    if (NTCAN_GET_TDC_SSPO(tdc_config) != 0) {
                        printf("Auto (TD:%d / SSPO: %d / SSPS: %d / TDF: %d)",
                            NTCAN_GET_TDC_TD(tdc_config), NTCAN_GET_TDC_SSPO(tdc_config),
                            NTCAN_GET_TDC_SSPS(tdc_config), NTCAN_GET_TDC_FILTER(tdc_config));
                    } else {
                        printf("Auto (TD: %d / SSPO:N/A / SSPS: %d / TDF: %d)",
                            NTCAN_GET_TDC_TD(tdc_config), NTCAN_GET_TDC_SSPS(tdc_config),
                            NTCAN_GET_TDC_FILTER(tdc_config));
                    }
                    break;
                case NTCAN_TDC_MODE_MANUAL:
                    printf("Manual (SSP: %d, TDF: %d)", NTCAN_GET_TDC_SSPS(tdc_config),
                        NTCAN_GET_TDC_FILTER(tdc_config));
                    break;
                case NTCAN_TDC_MODE_OFF:
                    printf("Disabled");
        ...
        if(0 == get_timestamp_freq(h0, &udtTimestampFreq, 0)) {
            ...
            printf("         TimestampFreq=%ld.%06ld / %ld.%06ld MHz",
                   (unsigned long)(udtTimestampFreq / 1000000),
                   (unsigned long)(udtTimestampFreq % 1000000),
                   (unsigned long)(info.sw_timestamp_freq / 1000000),
                   (unsigned long)(info.sw_timestamp_freq % 1000000));
            if (NTCAN_SUCCESS == ret) {
...
#ifdef NTCAN_IOCTL_GET_TX_TS_WIN
        if ((-3 == testnr) && (features & NTCAN_FEATURE_TIMESTAMPED_TX)) {
          uint32_t tx_ts_window;

          ret = canIoctl(h0, NTCAN_IOCTL_GET_TX_TS_WIN, &tx_ts_window);
          if (NTCAN_SUCCESS == ret) {
              printf("         TX-TS Window=%u ms", tx_ts_window);
              printf("\n");
          }
        }
#endif
        ...
        if(fwUpdateRequired != 0) {
            fwUpdateRequired = 0;   /* Reinitialize for next loop run. */
...
#ifdef NTCAN_FD
         "        txbuf rxbuf txtout rxtout baud[:dbaud:[tdc]] testcount data0 data1 ...]\n");
#else
...
static void print_cmsg(CMSG *pCmsg, CANTEST_CTX *pCtx)
{
    ...
    printf("%s=%9ld (0x%08lx) %s  dlc=%02X data= ",
            (pCmsg->id & NTCAN_EV_BASE) ? "EV" : "ID", (unsigned long)id,
            (unsigned long)id, pNtcanMetaData[meta], NTCAN_DLC(pCmsg->len));
...
static void print_cmsg_t(CMSG_T *pCmsgT, CANTEST_CTX *pCtx)
{
    ...
    if(pCtx->mode & NTCAN_MODE_OBJECT) {
        ...
        printf("ID= %4ld (0x%03lx) %s%c - %016" PRIx64 " - dlc=%02X  ",
