...
/* Only needed for really old kernels! */
...
#endif  /* (PLATFORM_64BIT) && (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,10)) */

int talgo = 2;
module_param(talgo, int, 0);
MODULE_PARM_DESC(talgo, "Select OSIF timer update algorithm (default: 2)");
...
#define BUILD_DATE_SIZE 32
        ...
        CAN_DPRINT((OSIF_ZONE_USR_INIT,
                    "esd CAN driver: " ESDCAN_DRIVER_NAME " init start\n"));
        CAN_DPRINT((OSIF_ZONE_USR_INIT,
...
#endif
        CAN_DPRINT((OSIF_ZONE_USR_DBG,
                    "esd CAN driver: object sizes: ULL %lu, UL %lu, UI %lu, US %lu;"
                    " sizeof jiffies %lu\n",
...
static VOID esdcan_show_done_info( VOID )
{
        CAN_DPRINT((OSIF_ZONE_USR_INIT, "esd CAN driver: " ESDCAN_DRIVER_NAME " successfully loaded\n"));
...
static VOID esdcan_show_card_info_all( VOID )
{
        ...
        for ( i = 0; i < MAX_CARDS; i++ ) {
                ...
                CAN_DPRINT((OSIF_ZONE_USR_INIT, "esd CAN driver: Card = %d\n", crd->card_no));
...
# endif /* #ifdef LTT */
        ...
        OSIF_DETACH();
        CAN_DPRINT((OSIF_ZONE_USR_INIT, "esd CAN driver: " ESDCAN_DRIVER_NAME " unloaded\n"));
        CAN_DBG((ESDCAN_ZONE_INIFU, "%s: leave\n", OSIF_FUNCTION));
}
...
OSIF_RCSID("$Id: esdcan.c 17291 2022-06-08 17:43:06Z stefanm $");
