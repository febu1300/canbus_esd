 ...
 * and the flags and TDC configuration from CAN_BAUDRATE_X
 */
typedef struct {
    UINT32  arbBTR;
    UINT32  dataBTR;
...
/*
 * Common entries implemented in module 'boardrc.c'
 */
UINT32 boardrc_fpga_type_get(CAN_CARD *pCrd);
VOID   boardrc_ts_set(CAN_NODE *pNode, OSIF_TS *pTs);
VOID   boardrc_tsfreq_get(CAN_NODE *pNode, OSIF_TS_FREQ *pFreq);
VOID   boardrc_corefreq_get(CAN_NODE *pNode, UINT32 *pFreq);
VOID   boardrc_timer_set(CAN_CARD *pCrd, OSIF_TS *pTs);
VOID   boardrc_timer_get(CAN_CARD *pCrd, OSIF_TS *pTs);
