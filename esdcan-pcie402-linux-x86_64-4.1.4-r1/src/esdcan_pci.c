 ...
 *           copyright 2005 - 2022 by esd electronics gmbh
 *
...
#if ( defined(BOARD_BUSMASTER) || defined(BOARD_MSI) ) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,12))
 ...
 *       please update to the next kernel-release (2.6.10 or following). */
        ...
        if ( 0 >= esdcan_pci_attached_cards ) {
                CAN_PRINT(("esd CAN driver: " ESDCAN_DRIVER_NAME " Version: %d.%d.%d\n", LEVEL, REVI, BUILD));
                CAN_PRINT(("esd CAN driver: " ESDCAN_DRIVER_NAME " No esd CAN card found!\n"));
