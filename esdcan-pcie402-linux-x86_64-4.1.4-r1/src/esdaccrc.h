/* -*- esdcan-c -*- */
/* FILE NAME esdaccrc.h
 *           copyright 2013 - 2022 by esd electronics gmbh
 *
...
/************************************************************************
 *
 *  Copyright (c) 2013 - 2022 by esd electronics gmbh
 *
...
/*
 * Defines to be used with overview module's registers
 */
/* ---- overview version register @0x04 */
...
#define FEATURE_MASK_DAR                (1 << FEATURE_IDX_DAR)
#define FEATURE_MASK_EXTEND             (1 << FEATURE_IDX_EXTEND)

/* Extended feature flags register @0x50 */
#define FEATURE_EXT_IDX_TX_PAUSE        0

#define FEATURE_EXT_MASK_TX_PAUSE       (1 << FEATURE_EXT_IDX_TX_PAUSE)

/* ---- overview info register @0x08 */
#define REG_OV_INFO_NUM_CORES_ANY(reg)  (UINT8)((reg) & 0x000000FF)
#define REG_OV_INFO_NUM_ACTIVE_CAN(reg) (UINT8)(((reg) & 0x0000FF00) >> 8)
...
/* ---- overview LIN info / config register @0x78 */
#define REG_OV_LIN_INFO_NUM(reg)        (UINT8)((reg) & 0x000000FF)
#define REG_OV_LIN_INFO_NUM_ACTIVE(reg) (UINT8)(((reg) & 0x0000FF00) >> 8)
#define REG_OV_LIN_INFO_MASTER_SEL_MASK(reg, node) ((reg) & (1 << (16 + node)))
#define REG_OV_LIN_INFO_REG_IDX(reg)    (UINT8)(((reg) & 0xFF000000) >> 24)

/* ---- overview mode register @0x2C */
...
#define ESDACCRC_OVERVIEW_PARTITION_1                           \
        ...
        UINT32          p1_reserved1[2];   /* 0x0048-0x004C */  \
        volatile UINT32 featureExt;        /* 0x0050 */         \
        volatile UINT32 modeExt;           /* 0x0054 */         \
        UINT32          p1_reserved2[6];   /* 0x0058-0x006C */  \
        volatile UINT32 bmIrqCnt;          /* 0x0070 */         \
        volatile UINT32 bmIrqMask;         /* 0x0074 */         \
        volatile UINT32 linInfoCfg;        /* 0x0078 */         \
        UINT32          p1_reserved3[1];   /* 0x007C */         \
        volatile UINT32 msiData;           /* 0x0080 */         \
        volatile UINT32 msiAddrOffs;       /* 0x0084 */         \
        UINT32          p1_reserved4[478]
/* Pad to end of 2kB or 512 longwords per "partition"/"sub unit" */
