...
#undef ESDACC_RELEASE_RX_TSH            /* define to use RX FIFO release on TSH in non-BM mode */
#undef ESDACC_UNROLL_RX_READ            /* define to unroll CAN-FD RX data read in non-BM mode */
#undef ESDACC_USE_NO_HW_TIMER           /* Do use the HW timers of esdACC */
...
#       define ESDACC_BM_STATISTIC      /* works only with BUSMASTER enabled! */
#       define ESDACC_MEASURED_BUSLOAD  /* works only with BUSMASTER enabled! */
#       define ESDACC_BM_TEST           /* has boardrc_bm_test routine */
#else
/* All this stuff needs BUSMASTER enabled */
#       undef ESDACC_BM_STATISTIC
#       undef ESDACC_MEASURED_BUSLOAD
#       undef ESDACC_BM_TEST
#endif
#undef ESDACC_FPGA_IO

#include <esdaccrc.h>  /* Has to be included BEHIND those config defines above */

/* TODO: remove HARDTS define (still used in VxWorks and Win esdcan layers...) */
#define HARDTS
...
#define MAX_CARDS                       16
#define NODES_PER_CARD                  8
#define EEI_MAX_UNITS                   16 /* Available in CPCI serial */
...
struct _FPGA_SYSTEM {
        ...
        UINT32          reserved8[0x3800];
        PCIE_EP         pcieEP;            /* 0x10000: PCIe endpoint IP core */
        UINT32          pad_1_4000[0x10];
        SPI_CTRL        spiCtrl;           /* 0x14040: SPI control */
        UINT32          pad_1_4040[2];
        SYSTEM_ID       systemId;          /* 0x14060: System ID */
        UINT32          pad_1_4060[6];
#if defined(ESDACC_BM_TEST)
...
#else
        UINT32          pad_1_4080[0x3E0];
#endif  /* #if defined(ESDACC_BM_TEST) */
#if defined(ESDACC_IRIG_B)
        /* IRIG-B module */
        ...
        volatile INT8   irigBYearMode;     /* 0x15900 */
        volatile INT8   irigBYear;         /* 0x15901 */
        INT8            reserved6[2];      /* 0x15902 - 0x15903 */
        volatile UINT32 irigBTimeStart;    /* 0x15904 */
        volatile UINT32 irigBSyncMode;     /* 0x15908 */ /* Write 0x0001 to increase tolerance */
        /* UINT32       reserved11[0x1BD];    2kB or 512 longwords per "partition"/"sub unit" */
#endif  /* if defined(ESDACC_IRIG_B) */
};
...
#define CIF_CARD                                   \
        ...
        SPI_CTRL       *pBaseSpi;                  \
        PCIE_EP        *pBasePcieEP;               \
        UINT32          mskCoresIrq;    /* bit mask of active cores */  \
...
#define STRAPPING_MASK_ADDON_STRAPS_0_1 (STRAPPING_MASK_ADDON_STRAP_0 | STRAPPING_MASK_ADDON_STRAP_1)
