...
/************************************************************************
 *
 *  Copyright (c) 1996 - 2022 by esd electronics gmbh
 *
...
/*  Starting with some kernel version "modversions.h" is empty. Later it vanishes. TODO: Find correct kernel version. */
#   warning "Ignoring <config/modversions.h>."
#  else
...
/* The Linux internal timer implementation changed so that timers with longer expiry
 * delta time expire with a granularity greater than the jiffie tick. Adjust the
 * OSIF timer handling to keep an expiry granularity of one jiffie tick.
 */
/* GIT Hash on mainline kernel: 500462a9de657f86edaa102f8ab6bff7f7e43fc2 */
#define OSIF_TIMER_FAST_WHEEL 1

/* Set this != 0 to get some runtime checks printing warnings. */
#define LXT_DEBUG       0
...
struct _OSIF_LINUX_TIMER {
        ...
        OSIF_DPC            dpc;
#if defined(OSIF_TIMER_FAST_WHEEL)
        unsigned long       real_tgt;
        unsigned long       cur_tgt;
#if LXT_DEBUG != 0
        unsigned int        mod_cnt;
#endif
#endif  /* #if defined(OSIF_TIMER_FAST_WHEEL) */
};
...
INT32 OSIF_CALLTYPE osif_memcpy(VOID *dst, const VOID *src, UINT32 size)
{
        memcpy(dst, src, size);
        return OSIF_SUCCESS;
}

INT32 OSIF_CALLTYPE osif_memcmp(const VOID *dst, const VOID *src, UINT32 size)
{
...
/*
 * The following stuff emulates the pci_*_consistent() API from the
 * former pci-dma-compat.h which vanishes around kernel 5.17.0.
 * Emulated / wrapped functions:
 *      pci_set_consistent_dma_mask()
 *      pci_alloc_consistent()
 *      pci_free_consistent()
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,4,0)
/* Something like pci_set_consistent_dma_mask() was not implemented in early kernels. */
static inline int _osif_set_consistent_dma_mask(struct pci_dev *pcidev, u64 mask)
{
	return 0;
}

static inline void *_osif_alloc_consistent(struct pci_dev *pcidev, size_t size,
...
#define _osif_free_consistent(cookie, size, ptr, dma_ptr) kfree(ptr)

#elif LINUX_VERSION_CODE < KERNEL_VERSION(2,6,34)
static inline int _osif_set_consistent_dma_mask(struct pci_dev *pcidev, u64 mask)
{
#if  (LINUX_VERSION_CODE >= KERNEL_VERSION(2,5,71))
        /* First occurence of a pci_set_consistent_dma_mask(). */
        return pci_set_consistent_dma_mask(pcidev, mask);
#else
        return 0;
#endif        
}
...
static inline void *
_osif_alloc_consistent(struct pci_dev *pcidev, size_t size,
                       dma_addr_t *dma_handle)
{
...
static inline void
_osif_free_consistent(struct pci_dev *pcidev, size_t size,
...
#else
/* Since 2.6.34 the DMA map API is complete enough to be usable here. */
static inline int _osif_set_consistent_dma_mask(struct pci_dev *pcidev, u64 mask)
{
	return dma_set_coherent_mask(&pcidev->dev, mask);
}
...
_osif_alloc_consistent(struct pci_dev *pcidev, size_t size,
		     dma_addr_t *dma_handle)
{
	return dma_alloc_coherent(&pcidev->dev, size, dma_handle, GFP_ATOMIC);
}
...
#endif
/*
 *      End of pci-dma-compat.h emulation
 */
...
#endif
        if (_osif_set_consistent_dma_mask(pcidev, DMA_BIT_MASK(32))) {
               OSIF_PRINT(("esd CAN driver: No 32-Bit bus master buffer available.\n"));
               return OSIF_INSUFFICIENT_RESOURCES;
        }
        *vaddr = _osif_alloc_consistent(pcidev, size, &((*paddr)->sys_dma_addr));
        if ( NULL == *vaddr ) {
                vfree( *paddr );
                return OSIF_INSUFFICIENT_RESOURCES;
        }
...
{
        OSIF_DBG((OSIF_ZONE_OSIF, "%s: enter\n", OSIF_FUNCTION));
        _osif_free_consistent(pcidev, size, vaddr, (*paddr)->sys_dma_addr);
        vfree( *paddr );
        OSIF_DBG((OSIF_ZONE_OSIF, "%s: leave\n", OSIF_FUNCTION));
        return OSIF_SUCCESS;
}
...
# endif
        ...
        kthread_complete_and_exit(&dpcThreadCompletion, 0);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,0)
...
#if defined(OSIF_TIMER_FAST_WHEEL)
/* The multi granularity Linux timer implementation is active. */
/* Its number of granularity levels depends on HZ configuration. */
#if HZ > 100
# define MAX_LVL        9
# else
# define MAX_LVL        8
...
#define LVL_LOW_MSK(L)  ~((unsigned long)0U) << (3 + 3 * L)
static const unsigned long lvl_low_mask[MAX_LVL + 1] = {
        ~((unsigned long)0U),
        LVL_LOW_MSK(1),
        LVL_LOW_MSK(2),
        LVL_LOW_MSK(3),
...
/* Manually import <talgo> from esdcan.c, a module parameter that selects the
 * algorithm for the next timer step calculation. */
extern const unsigned int talgo;

/*! \fn    next_lx_timer_target(unsigned long cur, unsigned long tgt)
 *  \brief Calculate next timer expiry that can be reached with no overshoot over tgt
 *  \param cur  current jiffies
 *  \param tgt  requested timer end jiffies
 *  \returns    a timer expiry that can be reached with no overshoot over tgt
 *
 * This function calculates the next timer expiry according to Linux kernel timer
 * expiry level.
 */
static unsigned long next_lx_timer_target(unsigned long cur, unsigned long tgt)
{
        unsigned long nxt = tgt; /* Will return tgt if (cur >= tgt); algo 2 */
        ...
        case 2: /* level steps */
        {
                ...
                for (lvl = 0; cur < (tgt & lvl_low_mask[lvl]) && lvl < MAX_LVL; ++lvl) {
                        nxt = tgt & lvl_low_mask[lvl];
                }
        }
        break;
        case 1: /* fixed steps */
...
/*! \fn    timer_action(OSIF_LINUX_TIMER *pLxTimer)
 *  \brief Triggers a dpc for the user timer handler if appropriate
 *  \param pLxTimer OSIF_LINUX_TIMER argument
 *
 * If the target expiry time is reached it calls OSIF_DPC_TRIGGER().
 *
 * Depending on the Linux timer implementation either this can be done directly
 * or we need to reschedule the current timer to the next appropriate intermediate
 * timer expiry target until the <real_tgt> expiry time can be reached directly
 * without overshoot.
 */
static inline void timer_action(OSIF_LINUX_TIMER *pLxTimer)
{
#if defined(OSIF_TIMER_FAST_WHEEL)
        unsigned long cur_tgt, real_tgt, now = READ_ONCE(jiffies);

#if LXT_DEBUG != 0
        if (now > pLxTimer->real_tgt) {         // check last target only
        // if (now > pLxTimer->cur_tgt) {          // check intermediate targets also
                pr_warn_ratelimited("Late Timer: c 0x%lx, n 0x%lx, t 0x%lx\n",
                                    pLxTimer->cur_tgt, now, pLxTimer->real_tgt);
        }
        if (now < pLxTimer->cur_tgt) {
                pr_warn_ratelimited("Early Timer: c 0x%lx, n 0x%lx, t 0x%lx\n",
                                    pLxTimer->cur_tgt, now, pLxTimer->real_tgt);
        }
#endif  /* LXT_DEBUG != 0 */

        real_tgt = pLxTimer->real_tgt;
        if (now < real_tgt) {
                cur_tgt = next_lx_timer_target(now, real_tgt);
                pLxTimer->cur_tgt = cur_tgt;
#if LXT_DEBUG != 0
                ++pLxTimer->mod_cnt;    /* Debug: */
#endif
                mod_timer(&(pLxTimer->timer), cur_tgt - 1);
                return;
        }
#if LXT_DEBUG != 0
        pr_warn_ratelimited("DPC: a %u, mc %u, n 0x%lx, t 0x%lx\n",
                            talgo, pLxTimer->mod_cnt, now, pLxTimer->real_tgt);
#endif
#endif  /* #if defined(OSIF_TIMER_FAST_WHEEL) */
...
/*! \fn    void timer_stub(struct timer_list *pLxTimerList)
 *  \brief The timer stub runs on soft-irq-level, triggers a dpc for the user timer handler
 *  \param pLxTimerList Linux struct timer_list argument
 *
 * This Linux version dependent timer stub determines the OSIF_LINUX_TIMER of the user
 * timer handler and enters common timer_action() code.
 */
...
INT32 OSIF_CALLTYPE osif_timer_set( OSIF_TIMER *t, UINT32 ms )
{
        ...
        if (ms) {
                ...
                real_tgt = now + ms;
#else
                real_tgt = now + (((ms * HZ) + 500) / 1000); /* round correctly */
#endif
                cur_tgt = next_lx_timer_target(now, real_tgt);

                pLxTimer->real_tgt = real_tgt;
                pLxTimer->cur_tgt = cur_tgt;
#if LXT_DEBUG != 0
                pLxTimer->mod_cnt = 1U; /* Debug: First mod_timer() call follows. */
#endif
                mod_timer(&(pLxTimer->timer), cur_tgt - 1);

#else  /* #if defined(OSIF_TIMER_FAST_WHEEL) */
                unsigned long real_tgt;
#if HZ == 1000
                real_tgt = jiffies + ms;
#else
                real_tgt = jiffies + (((ms * HZ) + 500) / 1000); /* round correctly */
#endif
                mod_timer(&(pLxTimer->timer), real_tgt);
#endif  /* #if defined(OSIF_TIMER_FAST_WHEEL) */
        } else {
                del_timer(&(pLxTimer->timer));
        }
...
OSIF_RCSID("$Id: osif.c 17283 2022-06-03 21:30:53Z stefanm $");
