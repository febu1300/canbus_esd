 ...
 *           copyright 2002 - 2022 by esd electronics gmbh
 *
...
/************************************************************************
 *
 *            Copyright (c) 2002 - 2022 by esd electronics gmbh
 *
...
INT32 OSIF_CALLTYPE esdcan_tx_get( CAN_OCB *ocb, CM *cm ) {
        CAN_DBG((ESDCAN_ZONE_FU, "%s: enter (tx.cm_size=%u)\n", OSIF_FUNCTION, ocb->tx.cm_size));
        ...
        if (copy_from_user(cm, ocb->tx.user_buf, ocb->tx.cm_size)) {
                CAN_DBG((ESDCAN_ZONE_FU, "%s: copy_from_user(%u) failed (ocb=%x)\n", OSIF_FUNCTION, ocb->tx.cm_size, ocb));
                return OSIF_EFAULT;
        }
...
#ifdef BOARD_CAN_FD
static inline INT32 cmsg_to_user( CAN_OCB *ocb, UINT8 *dst, CAN_MSG_X *src )
{
        /* FD-enabled driver, src is CAN_MSG_X */
        if (ocb->rx.cm_size==sizeof(CAN_MSG_T)) {
                /* Need to copy CAN_MSG_X data to user-supplied CAN_MSG_T buffer */
                /* Make a local copy, to call copy_to_user() only ONCE */
                CAN_MSG_T tmp_t;

                memcpy(&tmp_t,src,sizeof(CAN_MSG));
                tmp_t.timestamp = src->timestamp;
                if (copy_to_user(dst, &tmp_t, sizeof(CAN_MSG_T))) {
                        return OSIF_EFAULT;
                }
        } else {
                if (copy_to_user(dst, src, ocb->rx.cm_size)) {
                        return OSIF_EFAULT;
                }
        }
        return OSIF_SUCCESS;
...
#else
static inline INT32 cmsg_to_user( CAN_OCB *ocb, UINT8 *dst, CAN_MSG_T *src )
{
        ...
        if (ocb->rx.cm_size==sizeof(CAN_MSG_X)) {
                ...
                if (copy_to_user(dst, &tmp_x, sizeof(CAN_MSG_X))) {
                        return OSIF_EFAULT;
                }
        } else {
                if (copy_to_user(dst, src, ocb->rx.cm_size)) {
...
#endif  /* #ifdef BOARD_FD */

/*! \fn esdcan_rx_put( CAN_OCB *ocb, CM *cm );
 *  \brief copy received CM into ocb's RX buffer or to user if possible
 *  \param ocb open control block for this driver instance
 *  \cm pointer to a CM structure that contains the received CAN message.
 *  The CAN_MSG is copied to the user directly if the callback is entered from
 *      a canTake*() call within user context.
...
{
        CAN_DBG((ESDCAN_ZONE_FU, "%s: enter\n", OSIF_FUNCTION));
        if (0 != (ocb->rx.state & RX_STATE_PENDING_TAKE)) {
                /* Synchronous call by canTake*() within user context -> copy to user. */
                INT32 result;

                CAN_DBG((ESDCAN_ZONE, "%s: user_buf %p, CM @%p\n", OSIF_FUNCTION,
                         ocb->rx.user_buf, cm));

#ifdef BOARD_CAN_FD
                result = cmsg_to_user(ocb, ocb->rx.user_buf, (CAN_MSG_X *)cm);
#else
                result = cmsg_to_user(ocb, ocb->rx.user_buf, (CAN_MSG_T *)cm);
#endif
                if (OSIF_SUCCESS != result) return result;

                ocb->rx.user_buf += ocb->rx.cm_size;
        } else {
                /* May be called from back-end without user context -> copy to shadow buffers */
                /* Attention: we only copy the CAN_MSG_[TX] part of the CM into the receive buffer */
...
NUC_CALLBACKS esdcan_callbacks = {
        esdcan_close_done,
        esdcan_tx_done,
        esdcan_tx_get,  /* esdcan_tx_obj_get() was implemented like esdcan_tx_get() */
        esdcan_tx_get,  /* so we use esdcan_tx_get() here instead of esdcan_tx_obj_get() */
...
/* returns error codes negative and copied messages positive */
static INT32 esdcan_shadow_cmsg_to_user( CAN_OCB *ocb, void *buf, UINT32 cm_count )
{
        ...
        src = ocb->rx.cmbuf;
        CAN_DBG((ESDCAN_ZONE, "%s: rx.cm_size=%u\n", OSIF_FUNCTION, ocb->rx.cm_size));
        for (cpy_cnt = 0U; OSIF_SUCCESS == result && cpy_cnt < cm_count; ++cpy_cnt) {
                CAN_DBG((ESDCAN_ZONE, "%s: dst=%p, src=%p\n", OSIF_FUNCTION, dst, src));
                result = cmsg_to_user(ocb, dst, src);
                CAN_DBG((ESDCAN_ZONE, "%s: ts=%lx.%lx\n", OSIF_FUNCTION,
                         ((OSIF_TS)src->timestamp).h.HighPart, ((OSIF_TS)src->timestamp).h.LowPart));
                ++src;
                dst += ocb->rx.cm_size;
        }
        return OSIF_SUCCESS == result ? (INT32)cpy_cnt : -result;
}

#ifdef OSIF_USE_PREEMPT_RT_IMPLEMENTATION
/* returns error codes negative and received messages positive */
static INT32 esdcan_read_common( CAN_OCB *ocb, void *buf, UINT32 cm_count, UINT32 objSize )
{
        INT32 result;
        ...
        result = nuc_rx(ocb, ocb->rx.tout, &cm_count);
        if (result != OSIF_SUCCESS) {
                ocb->rx.state = 0;
                return -result;
        }
        ...
        if ( wait_event_interruptible( ocb->wqRx,
                                       ( (ocb->rx.cm_count > 0U) ||                 /* Successful end of canRead() */
        ...
        result = esdcan_shadow_cmsg_to_user(ocb, buf, (UINT32)result);
        CAN_DBG((ESDCAN_ZONE_FU, "%s:IOCTL_read: leave, copied %d CM(SG)s\n", OSIF_FUNCTION, result));
        return result; /* return CM count and not count*size */
}
#else
/* returns error codes negative and received messages positive */
static INT32 esdcan_read_common( CAN_OCB *ocb, void *buf, UINT32 cm_count, UINT32 objSize )
{
        ...
        /* wait for rx finished (interruptible) */
        OSIF_MUTEX_UNLOCK(&ocb->node->lock);
        result = down_interruptible(&ocb->sema_rx);
        OSIF_MUTEX_LOCK(&ocb->node->lock);
        if (0 != result) {
                if (ocb->rx.state & RX_STATE_CLOSING) {
                        /* The ocb got closed already, freeing of OCB resources
                           is done in final close() */
                        CAN_DBG((ESDCAN_ZONE, "%s: RX delayed cleanup\n", OSIF_FUNCTION));
                        return -EINTR; /* system error code, never reaches user, thus no OSIF_ */
                } else if (ocb->rx.cm_count == 0U) {
...
static INT32 esdcan_write_common( CAN_OCB *ocb, void *buf, UINT32 cm_count, UINT32 objSize )
{
        INT32   result;
        UINT32  old_job_tx_pending = 0U;
        INT32   tx_len;
        INT32   flagInterruptedBySignal = 0;
        INT32   flagOcbClosing = 0;
        INT32   old_state;
        ...
        if ( (OSIF_SUCCESS != result) && (0U == cm_count) ) {
        ...
        if (wait_event_interruptible(ocb->wqTx,
                                     ( (0 < (INT32)(ocb->tx.cm_count - old_job_tx_pending)) ||
                                       (ocb->tx.state & TX_STATE_ABORT) ||
                                       (ocb->tx.state & TX_STATE_CLOSING) ) ) ) {
                OSIF_MUTEX_LOCK(&ocb->node->lock);
                if ( (ocb->tx.cm_count == old_job_tx_pending) &&
                     (!(ocb->tx.state & TX_STATE_ABORT)) &&
                     (!(ocb->tx.state & TX_STATE_CLOSING)) ) {
                        flagInterruptedBySignal = 1;
                        ocb->tx.state |= TX_STATE_SIGNAL_INTERRUPT;
        ...
        OSIF_MUTEX_LOCK(&ocb->node->lock);
        tx_len = (INT32)(ocb->tx.cm_count - old_job_tx_pending);
        ocb->tx.cm_count = 0U;
...
static INT32 esdcan_write_common( CAN_OCB *ocb, void *buf, UINT32 cm_count, UINT32 objSize )
{
        ...
        CAN_DBG((ESDCAN_ZONE_FU, "%s: enter cm_count=%u\n", OSIF_FUNCTION, cm_count));
        ...
        /* wait until job is finished or error occured */
        OSIF_MUTEX_UNLOCK(&ocb->node->lock);
        result = down_interruptible(&ocb->sema_tx);
        OSIF_MUTEX_LOCK(&ocb->node->lock);
                                 ...
                                 CAN_OCB *ocb )
{
        ...
        IOCTL_ESDCAN_TAKE_00:
        {
                ...
                /* reset RX buffer pointer */
                ocb->rx.user_buf = ioArg->buffer;
                ocb->rx.cm_size = objSize;
                ocb->rx.cmbuf_in = ocb->rx.cmbuf;
                ocb->rx.cm_count = 0U;
                cm_count = ioArg->size;
                ioArg->size = 0U;
                CAN_DBG((ESDCAN_ZONE, "%s: user_buf=%p, cmbuf=%p\n", OSIF_FUNCTION, ocb->rx.user_buf, ocb->rx.cmbuf));
                CAN_DBG((ESDCAN_ZONE, "%s: cm_count=%u, cm_size=%u\n", OSIF_FUNCTION, cm_count, objSize));
                if (ocb->mode & CANIO_MODE_OBJECT) {
                        UINT32 id;
                        ...
                        /* RX object mode has no rx_done(), see comment below. */
                        result = nuc_rx_obj(ocb, &cm_count, id);
                        if (result != OSIF_SUCCESS) {
                                ocb->rx.state = 0;
                                break;
                        }
                        ...
                        /* trigger RX for cm_count cm */
                        result = nuc_rx(ocb, 0, &cm_count);
                        if (result != OSIF_SUCCESS) {
                                ocb->rx.state = 0;
                                break;
                        }
                        /* return if RX failed. ocb->rx.state is cleared by esdcan_rx_done(). */
                        if (0U == cm_count) {
                ...
                ocb->rx.state = 0;
                CAN_DBG((ESDCAN_ZONE, "%s: leave, copied %u CM(SG)s->%d\n", OSIF_FUNCTION, cm_count, result));
                ocb->rx.cm_count = 0U;  /* important to clear, for next read call */
                ioArg->size = cm_count; /* return CM count and not count*size */
                break;
        }
